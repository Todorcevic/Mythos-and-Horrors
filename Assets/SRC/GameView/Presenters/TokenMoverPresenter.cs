...
{
    public class TokneMover2Presenter : IEndReactionable
    {
        ...
        /*******************************************************************/
        TokneMover2Presenter(ReactionablesProvider _reactionablesProvider)
        {
            _reactionablesProvider.AddReactionable(this);
        }
        ...
        public async Task WhenFinish(GameAction gameAction)
        {
            ...
            {
                await GainHints(gainHintGameAction.Investigator, gainHintGameAction.FromCard, gainHintGameAction.Amount);
            }
            else if (gameAction is PayHintGameAction payHintGameAction)
            {
                await PayHints(payHintGameAction.Investigator, payHintGameAction.ToCard, payHintGameAction.Amount);
            }
        }

        /*******************************************************************/
        public async Task GainHints(Investigator investigator, Card fromCard, int amount)
        {
        ...
        public async Task PayHints(Investigator investigator, Card toCard, int amount)
        {
            TokenController hintsTokenController = _areaInvestigatorViewsManager.Get(investigator).HintsTokenController;
            List<TokenView> allTokens = _tokensGeneratorComponent.GetHintTokens(amount);
    ...
    public class TokenMoverPresenter : IResourceMover, IHintMover
    {
        [Inject(Id = ViewValues.CENTER_SHOW_POSITION)] private readonly Transform _centerShowPosition;
        [Inject] private readonly AreaInvestigatorViewsManager _areaInvestigatorViewsManager;
        [Inject] private readonly TokensGeneratorComponent _tokensGeneratorComponent;
