...
namespace MythsAndHorrors.PlayMode.Tests
{
    ...
    public class PositionTest : TestBase
    {
        ...
        public IEnumerator All_Zones_With_Cards()
        {
            ...
            yield return _cardMoverPresenter.MoveCardToZoneAsync(_chaptersProvider.CurrentScene.Info.Cards[0], _chaptersProvider.CurrentScene.PlotZone).AsCoroutine();
            yield return _cardMoverPresenter.MoveCardToZoneAsync(_chaptersProvider.CurrentScene.Info.Cards[1], _chaptersProvider.CurrentScene.GoalZone).AsCoroutine();
            yield return _cardMoverPresenter.MoveCardToZoneAsync(_chaptersProvider.CurrentScene.Info.Cards[3], _chaptersProvider.CurrentScene.DangerDeckZone).AsCoroutine();
            yield return _cardMoverPresenter.MoveCardToZoneAsync(_chaptersProvider.CurrentScene.Info.Cards[4], _chaptersProvider.CurrentScene.DangerDiscardZone).AsCoroutine();
            ...
            for (int i = 0; i < _chaptersProvider.CurrentScene.PlaceZone.GetLength(0); i++)
            {
                ...
                {
                    yield return _cardMoverPresenter.MoveCardToZoneAsync(_chaptersProvider.CurrentScene.Info.Cards[5 + k++], _chaptersProvider.CurrentScene.PlaceZone[i, j]).AsCoroutine();
                }
            }
