...
namespace MythosAndHorrors.PlayModeCORE1.Tests
{
    public class GameActionUpdateStatTests : TestCORE1Preparation
    {
        ...
        public IEnumerator GainAndPayHints()
        {
            ...
            yield return _gameActionsProvider.Create<GainHintGameAction>().SetWith(investigator, place.Hints, 2).Execute().AsCoroutine();

            Assert.That(investigator.Hints.Value, Is.EqualTo(2));

            yield return _gameActionsProvider.Create<PayHintGameAction>().SetWith(investigator, cardGoal.Hints, 1).Execute().AsCoroutine();
