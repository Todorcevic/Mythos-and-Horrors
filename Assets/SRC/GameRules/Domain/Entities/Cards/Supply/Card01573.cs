...
namespace MythosAndHorrors.GameRules
{
    public class Card01573 : CardSupply
    {
        ...
        private void Init()
        {
            CreateOptativeReaction<InvestigatePlaceGameAction>(Condition, Logic, GameActionTime.After, new Localization("OptativeReaction_Card01573"));
        }
        ...
        private async Task Logic(InvestigatePlaceGameAction investigatePlaceGameAction)
        {
            InteractableGameAction interactableGameAction = _gameActionsProvider.Create<InteractableGameAction>()
                .SetWith(canBackToThisInteractable: false, mustShowInCenter: true, new Localization("Interactable_Card01573"));

            foreach (Card itemCard in ControlOwner.DiscardZone.Cards.Where(card => card.HasThisTag(Tag.Item)))
            {
                interactableGameAction.CreateCardEffect(itemCard, new Stat(0, false), DrawItem, PlayActionType.Choose | PlayActionType.Draw, ControlOwner, new Localization("CardEffect_Card01573"));
